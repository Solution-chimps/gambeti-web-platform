...
<title>Treinamentos <?=$nr?> em Segurança do Trabalho</title>
</head>
<body>
...
// Executa uma consulta que pega cinco notícias
$sql = "SELECT * FROM textos WHERE tipo='treinamento' AND nr='$nr' ORDER BY visualizacao DESC, titulo ASC LIMIT $inicio,$qpp";
$query = $mysqli->query($sql);
while ($dados = $query->fetch_array()) {
	$titulo=$dados['titulo'];
	$texto=$dados['texto']; 
...
//prepara o rodape
	...
	$sql="SELECT COUNT(*) AS total FROM textos WHERE tipo='treinamento' AND nr='$nr'";
