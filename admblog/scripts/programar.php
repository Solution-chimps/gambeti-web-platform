...
if(($titulo=="")||($data=="")||($destaque=="")||($linhafina=="")||($texto=="")||($autor=="")||($tags=="")){
	...
	/*
	//alterar destaque
	$consultab="UPDATE noticiasprog SET destaque='0' WHERE destaque='$destaque'";
...
rename("../../uploads/$end","../../uploads/".$novoNome);
		...
		//else validacao tipo e tamanho do arquivo
			...
			$consulta="INSERT INTO noticiasprog (data, hora, titulo, autor, urlamigavel, linhafina, preview, destaque, identificador, autorurl) VALUES ('$dataformatada','$hora', '$titulo', '$autor', '$urlcriada', '$linhafina','$novoNome', '$destaque','$identificadornot','$autorurlcriada')";
			if($resultado=mysql_query($consulta)){
				...
				if($linha=mysql_fetch_assoc($resultado))
				{
					...
					//cria o txt do texto
					$fp = fopen("../../textosprog/".$idNoticiaCadastradaAgora.".txt" , "w");
					fwrite($fp, $texto);	
...
if($tags==""){
	...
	$consulta="INSERT INTO tagiadoprog (idnot, idtag) VALUES ('$idNoticiaCadastradaAgora', '$tag')";
			if($resultado=mysql_query($consulta)){
			}
			else{
				echo"<script>alert('Erro ao salvar tags da notícia. Por favor, tente novamente!');</script><script>history.go(-1);</script>";	
						...
						echo"<script>alert('Notícia programada com sucesso.');</script><script>window.location='../noticias.php';</script>";
				}
				else{
