...
if(($titulo=="")||($data=="")||($destaque=="")||($linhafina=="")||($texto=="")||($autor=="")||($tags=="")){
	...
	$consultab="UPDATE noticiasprog SET data='$dataformatada', autorurl='$autorurlcriada', hora='$hora', titulo='$titulo', autor='$autor', urlamigavel='$urlcriada', linhafina='$linhafina', destaque='$destaque' WHERE id='$id'";
	if($resultadob=mysql_query($consultab)){
		//edita o txt do texto
		$fp = fopen("../../textosprog/".$id.".txt" , "w");
		fwrite($fp, $texto);
	}
	else{
...
if($tags==""){
	...
	$consulta="INSERT INTO tagiadoprog (idnot, idtag) VALUES ('$id', '$tag')";
			if($resultado=mysql_query($consulta)){
			}
			else{
...
echo"<script>alert('Editado com sucesso');window.location='../editar-noticia-programada.php?id=$id';</script>";
