...
if($destaque!=0){
	...
	//	echo"Editei infos da OS<br />";
		...
		exit;
			...
			$consulta="INSERT INTO programasvoce (categoria, foto, video, nome, sinopse, data, hora, local, fotos, bttipo, indentificador, urlamigavel, destaque, ordem) VALUES ('$categoria', '$novo_nome', '$video', '$nome', '$sinopse', '$data', '$hora', '$local', '$enderecoalbum', '$tipobt', '$identificadornot', '$urlcriada', '$destaque', '$ordem')";
			if($resultado=mysql_query($consulta)){

				

			//carrega id desse cadastro
			$consultaz= "SELECT *FROM programasvoce WHERE indentificador='$identificadornot'";
		...
		$consultab="UPDATE eventosemdestaque SET tipo='para-voce', idprograma='$iddesseprograma' WHERE id='$destaque'";
		if($resultadob=mysql_query($consultab)){
		//	echo"Editei infos da OS<br />";
		}
		else{
			...
			exit;
				...
				for($c=1;$c<=$totalprodutos;$c++)
				{
					...
					if(($titulo=="")&&($texto=="")){
						...
						$consultaxxx="INSERT INTO textosvoce (idprograma, titulo, texto, ordem) VALUES ('$iddesseprograma', '$titulo', '$texto', '$ordem')";
						...
						echo"<script>alert('Cadastrado com sucesso.');</script><script>window.location='../programasvoce.php';</script>";
				}
				else{
