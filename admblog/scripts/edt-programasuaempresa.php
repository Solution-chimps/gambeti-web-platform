...
if($destaque!=0){
	...
	$consultab="UPDATE programassuaempresa SET categoria='$categoria', nome='$nome', video='$video', sinopse='$sinopse', data='$data', hora='$hora', local='$local', fotos='$enderecoalbum', bttipo='$tipobt', destaque='$destaque', urlamigavel='$urlcriada', ordem='$ordem' WHERE id='$id'";
	if($resultadob=mysql_query($consultab)){
	//	echo"Editei infos da OS<br />";
		//se foi colocado em destaque insiro na tb destaques
		$consultab="UPDATE eventosemdestaque SET tipo='para-sua-empresa', idprograma='$id' WHERE id='$destaque'";
		if($resultadob=mysql_query($consultab)){
		//	echo"Editei infos da OS<br />";
		}
		else{
...
$consulta="DELETE FROM textossuaempresa WHERE idprograma='$id'";
				...
				for($c=1;$c<=$totalprodutos;$c++)
				{
					...
					if($titulo=="DELETAR"){
						...
						if(($titulo=="")&&($texto=="")&&($ordem=="")){
							...
							$consultaxxx="INSERT INTO textossuaempresa (idprograma, titulo, texto, ordem) VALUES ('$id', '$titulo', '$texto','$ordem')";
							...
							echo"<script>alert('Editado com sucesso!');window.location='../programassuaempresa.php';</script>";
