...
<table id="tbconteudo" border="0" height="800" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <div id="chamadapagina"><br />&nbsp;&nbsp;Inscritos<br /><font size="2">&nbsp;&nbsp;&nbsp;Gerencie os inscritos nos cursos.</font></div>
        Lista de Inscritos
        <br /><br /><br />
...
?>  
<?php
			echo"<td align=\"right\"><div style=\"width:150px\" class=\"btlaranja\" onclick=\"window.location='scripts/exportar-inscritos.php?palavra=$palavra';\">Exportar Inscritos</div></td>";
?>
	...
	<tr>
    	<td align="right" height="40">
        	...
        	<form action="inscritos.php" method="get">
        	<div id="caixabusca">
            	<div id="btbusca"></div>
...
if(($palavra=="")&&($dtinicio!="")){
	...
	$contador="0";
	$consulta= "SELECT *FROM inscritos$busca ORDER BY id ASC LIMIT $inicio,$qpp";
	...
	$resultado = $mysqli->query($consulta) or die("<br />Falha na execução da consulta 2. Erro: " . $mysqli_error);	while ($linha = $resultado->fetch_assoc())
	{
		...
		if($formapg!="PagSeguro"){
			if($statuspg=="Aprovado"){
				...
				$btstatus.="<div class=\"btcinza\" onclick=\"mpg('$id')\">Marcar como Pago</div>";				
			}
...
echo"
	<tr class=\"linhacomumpar\">
    	<td class=\"celularcomum\" align=\"center\" valign=\"middle\">$identificador</td>		
		...
		<td class=\"celularcomum\" align=\"center\" valign=\"middle\" style=\"border-right:none;\" width=\"140\">$btstatus<div class=\"btcinza\" onclick=\"window.location='infos-inscrito.php?id=$id';\">Informações</div></td>
    </tr>
";			
		}
		else{
...
echo"
	<tr>
		<td colspan=\"5\" align=\"center\">Nenhum encontrado...</td>
	</tr>
";
	}
	else{
...
<table align="right" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
    	<td>
        <font size="1">
        	<strong>Legenda dos Status:</strong><br /><br />
<strong>Aguardando Infos:</strong> O visitante fez sua inscrição porém não escolheu a forma de pagamento no PagSeguro. No caso de inscrições em dinheiro ou depósito em conta esse é o status padrão.<br />
<strong>Aguardando pagamento: </strong> O visitante já escolheu sua forma de pagamento no PagSeguro, estamos esperando a confirmação do pagamento.<br />
<strong>Paga:</strong> O PagSeguro já confirmou o pagamento da inscrição. No caso de inscrições em dinheiro ou depósito o administrador já deu baixa no pagamento.<br />
<strong>Verificar no PagSeguro:</strong> Tivemos algum problema com o pagamento do visitante. Verifique em sua conta do PagSeguro o status e o motivo do erro.<br />
<br />
...
//verifica total de anuncios na consulta solicitada
   $result = mysql_query("SELECT COUNT(*) as Regs FROM inscritos$busca");
...
<script>
	...
	function dlbanner(id){
		var answer = confirm("Deseja mesmo deletar esse inscrito?")
		if (answer){
			window.location = "scripts/dlinscrito.php?id="+id;
		}
		else{
		}
	}	
	function mpg(id){
		var answer = confirm("Deseja mesmo marcar como pago essa inscrição?")
		if (answer){
			window.location = "scripts/mpg.php?id="+id;
		}
		else{
		}
	}		
