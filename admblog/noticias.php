...
<table id="tbconteudo" border="0" height="800" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <div id="chamadapagina"><br />&nbsp;&nbsp;Notícias<br /><font size="2">&nbsp;&nbsp;&nbsp;Gerenciamento das notícias cadastradas no site</font></div>
        Lista de Notícias<br />


<?php
//confere se existem noticias programadas
	$consulta = mysql_query("SELECT COUNT(*) AS total FROM noticiasprog");
   	$mostrarTotalProgs = mysql_fetch_array($consulta);
	$mostrarTotalProgs = $mostrarTotalProgs['total'];	
	

	if($mostrarTotalProgs>="1"){
		echo"

		Você possui $mostrarTotalProgs <a href=\"noticias-programadas.php\">notícias programadas</a> no site.	
		";
	}
	else{
	}
...
<table width="96%" border="0" cellpadding="0" cellspacing="0">
    ...
    <?php
			...
			echo"<td align=\"right\"><div style=\"width:150px\" class=\"btlaranja\" onclick=\"window.location='adicionar-noticia.php';\">Adicionar Notícia</div></td>";
...
?>           
        	<form action="noticias.php" method="get" id="formfiltrardatas">
            <div id="btfiltrar"><button class="btlaranja">Filtrar</button></div>            
        	<div id="caixadatas">
            	<div id="btcalendario"> Final</div>
            	<div id="areatextocal"><input value="<?=$inicio?>" type="text" id="to" name="to" class="configcampocal" /></div>
            </div>
            <div id="caixadatas">
            	<div id="btcalendario"> Incial</div>
            	<div id="areatextocal"><input value="<?=$fim?>" type="text" id="from" name="from" class="configcampocal" /></div>                                
            </div>
            <?=$btlimpar?> 
...
<table class="tabelalistagem" width="96%" cellpadding="8" cellspacing="0" border="0">
	...
	<tr class="linhaindice">
    	<td class="celularindice" align="center" valign="middle">Data</td>
    	<td class="celularindice" align="center" valign="middle"><a href="?ordem=ab&qntpp=9999999999999999999">Aberturas</a></td>        
    	<td class="celularindice" align="center" valign="middle"><a href="?ordem=abu&qntpp=9999999999999999999">Visualizações Únicas</a></td>                
    	<td class="celularindice" align="center" valign="middle">Título</td>
    	<td class="celularindice" align="center" valign="middle">Autor</td>  
...
if(($dtfim!="")&&($dtinicio!="")){
	...
	if($palavra!=""){
		$busca=" WHERE titulo REGEXP '$palavra'$complemento OR autor REGEXP '$palavra'$complemento";
	}
...
if($_GET["ordem"]=="ab"){
$ordenar=" ORDER BY aberturas DESC";	
}
else if($_GET["ordem"]=="abu"){
$ordenar=" ORDER BY aberturasunicas DESC";	
}
else{
$ordenar=" ORDER BY id DESC";	
	...
	$contador="0";
	$consulta= "SELECT *FROM noticias$busca$ordenar LIMIT $inicio,$qpp";
	...
	$resultado = $mysqli->query($consulta) or die("<br />Falha na execução da consulta 2. Erro: " . $mysqli_error);	while ($linha = $resultado->fetch_assoc())
	{
		...
		$valortotalos=$valortotalos+$total;
			...
			$btstatus="<div class=\"btcinza\" onclick=\"deletarnot('$id')\">Deletar</div>";

		
		if($divisaor=="0"){
echo"
	<tr class=\"linhacomumpar\">
    	<td class=\"celularcomum\" align=\"center\" valign=\"middle\">$datatratada</td>
    	<td class=\"celularcomum\" align=\"center\" valign=\"middle\">$aberturas</td>		
    	<td class=\"celularcomum\" align=\"center\" valign=\"middle\">$aberturasunicas</td>				
    	<td class=\"celularcomum\" align=\"center\" valign=\"middle\">$titulo</td>
    	<td class=\"celularcomum\" align=\"center\" valign=\"middle\">$autor</td>
		<td class=\"celularcomum\" align=\"center\" valign=\"middle\" style=\"border-right:none;\" nowrap=\"nowrap\">$btstatus<div class=\"btcinza\" onclick=\"window.location='editar-noticia.php?id=$id';\">Editar</div></td>
    </tr>
";			
		}
		else{
...
echo"
	<tr>
		<td colspan=\"5\" align=\"center\">Nenhuma notícia encontrada. Utilize o botão acima para cadastrar.</td>
	</tr>
";
	}
	else{
...
//prepara o rodape
	...
	$consulta = mysql_query("SELECT COUNT(*) AS total FROM noticias$busca");
...
//verifica total de anuncios na consulta solicitada
   $result = mysql_query("SELECT COUNT(*) as Regs FROM noticias$busca");
...
echo"<div class=\"btspaginacaofim\" onclick=\"window.location='?i=$ultimaPagina&palavra=$palavra&qntpp=$qntpp&to=$inicioxxx2&from=$fimxxx2';\">Última</div>";
...
if($qpp=="9999999999999"){
	$qpptratado="todos";
	$btmostrar="<a style=\"color:#333;\" href=\"?palavra=$palavra&qntpp=20&to=$inicioxxx2&from=$fimxxx2\">Mostrar 20 por página</a>";
}
else{
...
<script>
	...
	function deletarnot(id){
		var answer = confirm("Deseja mesmo deletar essa notícia?")
		if (answer){
			window.location = "scripts/dlnoticia.php?id="+id;
		}
		else{
		}
	}	
