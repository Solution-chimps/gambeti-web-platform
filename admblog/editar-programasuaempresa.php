<?php
	...
	$id=$_GET["id"];	
$consulta= "SELECT *FROM programassuaempresa WHERE id='$id'";
...
<table id="tbconteudo" border="0" height="800" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <div id="chamadapagina"><br />&nbsp;&nbsp;Editar Programa<br /><font size="2">&nbsp;&nbsp;&nbsp;Preencha as informações abaixo e edite.</font></div>
        <br />
		<br />
        <form action="scripts/edt-programasuaempresa.php?id=<?=$id?>" method="post">
        ...
        Foto<br />
        <img src="../imagens/<?=$foto?>" class="fotoprincipal" /><br />
		<a href='#' onClick="window.open('verFotoProgramaEmpresa.php?id=<?=$id?>','TrocarFoto','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=400,height=200,top=25,left=25'); return false;">alterar</a><br /><br />
...
<br />
        ...
        Sinopse<br />
		<textarea name="sinopse" class="configtexto"><?=$sinopse?></textarea><br /><br />  


        Data<br />
		<input type="text" name="data" value="<?=$data?>" class="configcampo"><br /><br />  

        Hora<br />
		<input type="text" name="hora" value="<?=$hora?>" class="configcampo"><br /><br />  

        Local<br />
		<input type="text" name="local" value="<?=$local?>" class="configcampo"><br /><br />  
...
<div id="areaenderecofotos" style="display:none">
		...
		<input type="text" name="enderecoalbum" id="enderecoalbum" value="<?=$fotos?>" class="configcampo"><br /><br />  
</div>
...
//carrega todos produtos desse orcamento
	$consulta= "SELECT *FROM textossuaempresa WHERE idprograma='$id' ORDER BY ordem";
...
<?php
	if($fotos==""){
		...
		$exibirbt="Sim";		
		echo"document.getElementById(\"areaenderecofotos\").style.display=\"block\";";
	}
?>
		
		document.getElementById("categoria").value="<?=$categoria?>";
		document.getElementById("exibirbt").value="<?=$exibirbt?>";		
		document.getElementById("tipobt").value="<?=$bttipo?>";
